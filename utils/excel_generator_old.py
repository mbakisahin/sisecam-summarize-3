...
class ExcelReportGenerator:
    ...
    def adjust_first_four_column_width(self, worksheet):
        ...
        worksheet.column_dimensions['B'].width = 25  # Keyword
        worksheet.column_dimensions['C'].width = 15  # Date
        worksheet.column_dimensions['D'].width = 30  # Kaynak (Original Document linki)

    def add_neighbor_links_and_comparisons(self, worksheet, neighbor_urls, individual_comparisons):
        # Komşu PDF linklerini ve karşılaştırma metinlerini ekler
        for idx, (neighbor_url, comparison_text) in enumerate(zip(neighbor_urls, individual_comparisons), 1):
            ...
            # Karşılaştırma metnini ekleyin
            comparison_cell = worksheet.cell(row=2, column=5 + 2 * idx)
            comparison_cell.value = self.wrap_text(comparison_text)
            ...
            worksheet.column_dimensions[get_column_letter(5 + 2 * idx - 1)].width = 15
            worksheet.column_dimensions[get_column_letter(5 + 2 * idx)].width = 50
    ...
    def create_excel(self, metadata, file_name='comparison_report.xlsx'):
        ...
        data = {
            ...
            'Kaynak': [''],
            'Key Differences': [self.wrap_text(metadata.get('combined_comparison', 'N/A'))]
        }
        ...
        with pd.ExcelWriter(file_name, engine='openpyxl') as writer:
            ...
            # Komşu linkleri ve karşılaştırma metinlerini ekleyin
            self.add_neighbor_links_and_comparisons(worksheet, neighbor_urls, individual_comparisons)
